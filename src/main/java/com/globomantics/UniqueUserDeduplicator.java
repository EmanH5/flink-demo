...
public class UniqueAdClickAggregator {
  ...
  private static Properties ccloudProps() {
    // Pull Confluent Cloud creds from env (configure in Gitpod)
    ...
    p.setProperty("sasl.jaas.config",
        String.format("org.apache.kafka.common.security.plain.PlainLoginModule required username=\"%s\" password=\"%s\";",
            apiKey, apiSecret));
    // Recommended for some environments
  ...
  public static void main(String[] args) throws Exception {
    ...
    env.enableCheckpointing(10_000); // clip 6 goes deeper

    // ---- Source: Confluent Cloud Kafka (topic: ad-clicks) ----
    ...
    DataStream<ClickEvent> events = env
        .fromSource(source, WatermarkStrategy.noWatermarks(), "ad-clicks-source")
        .map(line -> {
    ...
    DataStream<Tuple2<String, Long>> perAdCounts = unique
        ...
        .process(new ProcessWindowFunction<ClickEvent, Tuple2<String,Long>, String, TimeWindow>() {
          ...
          public void process(String adId, Context ctx, Iterable<ClickEvent> it, Collector<Tuple2<String,Long>> out) {
            long c = 0;
            for (ClickEvent ignored : it) c++;
            out.collect(Tuple2.of(adId, c));
          }
        });

    // ---- Sink: Confluent Cloud Kafka (topic: ad-agg) ----
